...
public class ReactiveForwarding {
    ...
    @Property(name = "startCheck", boolValue = false,
            label = "Enable packet-out only forwarding; default is false")
    ...
    long [] pByte = new long[4];
    long [] pPacket = new long[4];

    HashMap<String, Integer> src_check = new HashMap<>();
    HashMap<String, MacAddress> [] mac_check = new HashMap[5];
    HashMap<String, MacAddress> mac_check2 = new HashMap<>();
    HashMap<String, Double> flow_sp = new HashMap<>();
    HashMap<String, Long> pre_byte = new HashMap<>();
    HashMap<String, PortNumber> in_port = new HashMap<>();

    int [] flowArray = new int [] {0, 0, 0, 0};
    ...
    private void readComponentConfiguration(ComponentContext context) {
        ...
        Boolean startCheckEnabled =
                Tools.isPropertyEnabled(properties, "startCheck");
        if (packetOutOnlyEnabled == null) {
            log.info("Packet-out is not configured, " +
                             "using current value of {}", startCheck);
        } else {
            startCheck = startCheckEnabled;
            log.info("Configured. Packet-out only forwarding is {}",
                     startCheck ? "enabled" : "disabled");
        }
...
//            installRule(context, path.src().port(), macMetrics);
            String s1 = "of:0000000000000001";
            if (s1.equals(context.inPacket().receivedFrom().deviceId().toString()) && startCheck == true) {
                int src = src_check.get(context.inPacket().parsed().getSourceMAC().toString());
                if (mac_check[src].containsValue(context.inPacket().parsed().getDestinationMAC()) == false) {
                    installRule(context, pickPortNumber(), macMetrics);
                    mac_check[src].put(context.inPacket().parsed().getDestinationMAC().toString(), context.inPacket().parsed().getDestinationMAC());
                    mac_check2.put(context.inPacket().parsed().getDestinationMAC().toString(), context.inPacket().parsed().getDestinationMAC());
                }
            }
            else
                installRule(context, path.src().port(), macMetrics);
...
//                && (inPkt.getSourceMAC().toString().equals("00:00:00:00:00:01")
//                || inPkt.getSourceMAC().toString().equals("00:00:00:00:00:02")
//                || inPkt.getSourceMAC().toString().equals("00:00:00:00:00:03")
//                || inPkt.getSourceMAC().toString().equals("00:00:00:00:00:04")
//                || inPkt.getSourceMAC().toString().equals("00:00:00:00:00:05")) && (startCheck == true)) {
//            mac_check.put(inPkt.getSourceMAC().toString(), inPkt.getSourceMAC());
//            mac_check.put(inPkt.getDestinationMAC().toString(), inPkt.getDestinationMAC());
//            flow_sp.put(inPkt.getSourceMAC().toString() + inPkt.getDestinationMAC().toString(), 0.0);
//            pre_byte.put(inPkt.getSourceMAC().toString() + inPkt.getDestinationMAC().toString(), 0l);
//            in_port.put(inPkt.getSourceMAC().toString() + inPkt.getDestinationMAC().toString(), context.inPacket().receivedFrom().port());
//        }
    ...
    // 만든것
    private PortNumber pickPortNumber() {
        if (flowArray[0] <= 59 || flowArray[1] <= 59) {
            if (flowArray[0] == flowArray[1]) {
                flowArray[0]++;
                return PortNumber.portNumber(6);
            } else if (flowArray[0] > flowArray[1]) {
                flowArray[1]++;
                return PortNumber.portNumber(7);
            } else if (flowArray[0] < flowArray[1]) {
                flowArray[0]++;
                return PortNumber.portNumber(6);
            }
        } else {
            if (flowArray[2] == flowArray[3]) {
                flowArray[2]++;
                return PortNumber.portNumber(8);
            } else if (flowArray[2] > flowArray[3]) {
                flowArray[3]++;
                return PortNumber.portNumber(9);
            } else if (flowArray[2] < flowArray[3]) {
                flowArray[2]++;
                return PortNumber.portNumber(8);
            }
...
//        mac_check.put(inPkt.getSourceMAC().toString(), inPkt.getSourceMAC());
    ...
    public class Task extends TimerTask {
        ...
        public void run() {
            log.info("timer");
            DeviceId id = DeviceId.deviceId("of:0000000000000001");
            if (id != null) {
//                Iterable<FlowEntry> flow = flowRuleService.getFlowEntries(id);
//                double alpha = 0.125;
//                for (FlowEntry f : flow) {
//                    MacAddress mac_dst = null, mac_src = null;
//                    Iterator<Criterion> cri_temp = f.selector().criteria().iterator();
//                    while (cri_temp.hasNext()) {
//                        Criterion c = cri_temp.next();
//                        if (c.type() == Criterion.Type.ETH_DST) {
//                            mac_dst = ((EthCriterion) c).mac();
//                        } else if (c.type() == Criterion.Type.ETH_SRC) {
//                            mac_src = ((EthCriterion) c).mac();
//                        }
//                    }
//                    if (mac_src != null) {
//                        if ((mac_src.toString().equals("00:00:00:00:00:01")
//                                || mac_src.toString().equals("00:00:00:00:00:02")
//                                || mac_src.toString().equals("00:00:00:00:00:03")
//                                || mac_src.toString().equals("00:00:00:00:00:04")
//                                || mac_src.toString().equals("00:00:00:00:00:05")) && startCheck == true) {
//                            String s = mac_src.toString() + mac_dst.toString();
//                            long b = f.bytes() - pre_byte.get(s);
//                            double sp = (1 - alpha) * flow_sp.get(s) + alpha * b;
//                            flow_sp.put(s, sp);
//                            pre_byte.put(s, f.bytes());
//                        }
//                    }
//                }
//                List<Map.Entry<String, Double>> list_entris = new ArrayList<Map.Entry<String, Double>>(flow_sp.entrySet());
//
//                Collections.sort(list_entris, new Comparator<Map.Entry<String, Double>>(){
//                    public int compare(Map.Entry<String, Double> obj1, Map.Entry<String, Double> obj2) {
//                        return obj2.getValue().compareTo(obj1.getValue());
//                    }
//                });
//                int cnt = 0;
//                for (Map.Entry<String, Double> entry : list_entris) {
//                    MacAddress mac_dst, mac_src;
//                    mac_src = MacAddress.valueOf(entry.getKey().toString().substring(0, 17));
//                    mac_dst = MacAddress.valueOf(entry.getKey().toString().substring(17));
//                    if (cnt  < 120) {
//                        if (cnt%2 == 0)
//                            flowModify(id, in_port.get(entry.getKey()), PortNumber.portNumber(6), mac_src, mac_dst);
//                        else if (cnt%2 == 1)
//                            flowModify(id, in_port.get(entry.getKey()), PortNumber.portNumber(7), mac_src, mac_dst);
//                    }
//                    else {
//                        if (cnt%2 == 0)
//                            flowModify(id, in_port.get(entry.getKey()), PortNumber.portNumber(8), mac_src, mac_dst);
//                        else if (cnt%2 == 1)
//                            flowModify(id, in_port.get(entry.getKey()), PortNumber.portNumber(9), mac_src, mac_dst);
//                    }
//                    cnt += 1;
//                }

                PortStatistics [] p  = new PortStatistics[4];
                for (int i = 0; i < 4; i++)
                    p[i] = deviceService.getStatisticsForPort(id, PortNumber.portNumber(i+6));

                if (startCheck == true) {
                    if (count_check % 5 == 0) {
                        try {
                            FileWriter result = new FileWriter("/home/sdn/result/" + count_check + ".txt");
                            if (count_check == 5) {
                                for (int i = 0; i < 4; i++) {
                                    result.write("" + pByte[i] + "\t");
                                }
                            }
                            result.write("\n");
                            for (int i = 0; i < 4; i++) {
                                pByte[i] = p[i].bytesSent();
                                pPacket[i] = p[i].packetsSent();
    ...
    private void flowModify(DeviceId id, PortNumber in_port, PortNumber out_port, MacAddress src, MacAddress dst) {
        TrafficSelector.Builder selectorBuilder = DefaultTrafficSelector.builder();
        ...
        flowObjectiveService.forward(id, forwardingObjective);
    }
    ...
    private class InternalFlowListener implements FlowRuleListener {
        ...
        public void event(FlowRuleEvent event) {
            ...
            if (event.type() == RULE_REMOVED && flowRule.appId() == appId.id()) {
                MacAddress src = null, dst = null;
                Criterion c = flowRule.selector().getCriterion(Criterion.Type.ETH_DST);
                if (c != null) {
                    for (Criterion cr : flowRule.selector().criteria()) {
                        if (cr.type() == Criterion.Type.ETH_DST) {
                            dst = ((EthCriterion) cr).mac();
                        } else if (cr.type() == Criterion.Type.ETH_SRC) {
                            src = ((EthCriterion) cr).mac();
